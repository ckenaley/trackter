...
*trackter* also contains several functions for image and video processing. These functions depend on the popular `FFmpeg` package and it must be installed if the user intends to use them. Installation is platform-dependent. The [`FFmpeg` wiki installation and compilation guide](https://trac.ffmpeg.org/wiki/CompilationGuide) provides some great guidance here.
