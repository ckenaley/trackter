...
Here, the frames from a video of a swimming juvenile rainbow trout loaded with the package is are extracted with `video.to.images()` and saved to an appropriately named directory.
...
Using `gg.overlay()` users can plot a single frame of data or series of frames as a facetted plot. The function takes tables returned by the `kin` functions to plot contours and midline, head, or trailing edge data. In addition, `gg.overlay()` can access the results from `fin.kin()` to plot the position of fin-like extension. Here, the midline results for the first full tailbeat cycle is projected over the smoothed body contours is produced with `gg.overlay()`. Notice `under="cont.sm"` and `over="midline"` which specify the smooth contour as the underlying data and midline as the overlay data. 

```{r ggoverlay,eval=FALSE}
