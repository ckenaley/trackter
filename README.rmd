...
Next, a filter is designated that rescales the images extracted from the video with `video.to.images2`which is then called to perform the extraction. This filter rescales the images to 600 pixels wide, maintaining the aspect ratio. %Users may want to consult `FFmpeg` documentation for filtering video data at https://ffmpeg.org/ffmpeg-filters.html.
...
```{r}
 kin <- trackter::kin.simple(image.dir = "images",thr=0.6,ant.per = 0.15,show.prog=F)
```